    ...
    <StoreContext.Provider value={store}>
      <CompareSliderContext.Provider value={contextValue}>
        <RootPrimitive
          ...
          tabIndex={0}
          className={cn(
            "focus-visible:border-ring focus-visible:ring-ring/50 relative isolate touch-none overflow-hidden transition-all outline-none select-none focus-visible:ring-[3px]",
    ...
    >
      {children ?? (
        <>
          ...
          />
          {interaction === "drag" && (
            <div className="bg-background [&_svg]:text-muted-foreground z-50 flex aspect-square size-11 shrink-0 items-center justify-center rounded-full p-2 [&_svg]:size-4 [&_svg]:stroke-3 [&_svg]:select-none">
              {isVertical ? (
...
function CompareSliderLabel(props: CompareSliderLabelProps) {
  ...
  return (
    <LabelPrimitive
      ...
      className={cn(
        "border-border bg-background/80 absolute z-20 rounded-md border px-3 py-1.5 text-sm font-medium backdrop-blur-sm",
        isVertical
          ? side === "before"
            ? "top-2 left-2"
            : "bottom-2 left-2"
