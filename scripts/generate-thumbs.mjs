...
const HELP = `generate-thumbs — recursively create -thumb.webp variants from webp images
...
const { folder, flags } = await resolveCli(
  ...
  { bool: ["--force", "--dry-run"], value: ["--thumb-width", "--thumb-quality", "--thumb-suffix"] },
  HELP,
);
...
console.log(`Generating thumbnails in "${folder}"${flags.dryRun ? " (dry-run)" : ""}\n`);
